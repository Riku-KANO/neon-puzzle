...
import init, { generate_level, check_connection_status } from '../pkg/neon_puzzle';
...
function App() {
  ...
  const handleRotate = useCallback((x: number, y: number) => {
    ...
    setBoardData((prev) => {
      ...
      // Check win
      const targetIndex = (newBoard.height - 1) * newBoard.width + (newBoard.width - 1);
  ...
  if (!isWasmReady) return <div className="text-white flex items-center justify-center h-screen bg-black">Loading System...</div>;
  ...
  return (
    <div className="w-full h-screen bg-gray-950 text-white relative select-none">
      ...
      <div className="absolute top-4 left-4 z-10 pointer-events-none">
        ...
        {gameState === 'PLAY' && (
            <div className="bg-black/50 p-4 rounded-lg border border-cyan-500/30 backdrop-blur-sm">
                <p className="text-cyan-300">STATUS: <span className="text-white">CONNECTED</span></p>
                <p className="text-xs text-gray-400">Rotate blocks to route power.</p>
            </div>
        )}
        {gameState === 'WON' && (
             <div className="bg-black/80 p-6 rounded-xl border border-green-500 shadow-[0_0_30px_rgba(0,255,0,0.5)]">
                <h2 className="text-4xl font-bold text-green-400 text-center mb-2">SYSTEM RESTORED</h2>
                <button 
      ...
      <Canvas shadows>
        ...
        */}
        {gameState !== 'MENU' && (
             <OrbitControls ref={controlsRef} target={[0, 0, 0]} maxPolarAngle={Math.PI / 2.5} enablePan={false} />
        )}

        {/* Cinematic Camera for Menu */}
        {gameState === 'MENU' && (
             <OrbitControls autoRotate autoRotateSpeed={0.5} enableZoom={false} enablePan={false} minPolarAngle={Math.PI/3} maxPolarAngle={Math.PI/2} />
        )}
        
        <ambientLight intensity={0.2} />
        <spotLight position={[5, 10, 5]} angle={0.5} penumbra={1} intensity={10} castShadow color="#ccffff" />
        ...
        <group>
            ...
            {(gameState === 'PLAY' || gameState === 'WON') && boardData && (
                <group>
                    <Board data={boardData} onRotate={handleRotate} connectedIndices={connectedIndices} />
